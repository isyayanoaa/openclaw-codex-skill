...
当用户需要用 Codex 写代码时，按以下流程操作：

### 第一步：打开新的 iTerm2 终端

用 AppleScript 打开全新 iTerm2 窗口：

```bash
osascript -e 'tell application "iTerm2" to create window with default profile'
```

或通过 exec 工具在新终端 session 里操作（pty=true）。
...
> `proxy` 是用户封装的 alias，执行后当前 shell 会设置代理环境变量，使 Codex CLI 能访问 OpenAI API。
...
**Plan Mode 的意义：**
- Codex 只规划、不执行，双方先对齐方案
- 可以反复讨论、修改计划，直到满意
- 确认后再退出 plan mode，Codex 才开始实际写代码
- 避免写到一半发现方向不对，浪费 token 和时间

### 第五步：在 Plan Mode 中描述任务
...
计划确认后，退出 plan mode（直接继续对话即可），Codex 开始编写代码。
...
```
1. osascript 打开 iTerm2 新窗口
2. 输入 proxy（开启代理）
3. cd 进入项目目录
4. codex（启动 TUI）
5. /plan（进入计划模式）
6. 描述需求，与 Codex 对齐方案
7. 确认后退出 plan mode → Codex 开始写代码
8. 用 /diff 检查改动，/review 做最终审查
```
